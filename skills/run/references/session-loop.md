...
the pipeline's manifest declares `runner: session`. In this mode the main Claude
...
project, never under `${CLAUDE_PLUGIN_ROOT}`.
...
`Read` `${CLAUDE_PLUGIN_ROOT}/agents/pipeline-manager.md` and follow its
